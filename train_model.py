...
file_path = "dataset/Arabic Sign Language Letters Dataset.csv"

print("⏳ جاري قراءة الداتاسيت الشاملة وتنظيف البيانات...")
...
try:
    ...
    X_raw = df.drop(columns=['letter'])
    X = X_raw.iloc[:, :42].values 

    print("-" * 30)
    print(f" تم تحميل البيانات بنجاح.")
    print(f" إجمالي عينات التدريب: {len(X)}")
    print(f" عدد الحروف المكتشفة: {len(np.unique(y))}")
    print(f" قائمة الحروف: {list(np.unique(y))[:31]}") 
    ...
    print("-" * 30)
    print(" مبروك! الموديل الجديد اتدرب على الداتاسيت الشاملة وجاهز للعمل.")
